...
static const u_char *zbaselib_md5_body(zbaselib_md5_t *ctx, const u_char *data,
    size_t size);


void
zbaselib_md5_init(zbaselib_md5_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
zbaselib_md5_update(zbaselib_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        size -= free;
        (void) zbaselib_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = zbaselib_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }
...
void
zbaselib_md5_final(u_char result[16], zbaselib_md5_t *ctx)
{
    size_t  used, free;
...
static const u_char *
zbaselib_md5_body(zbaselib_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;
