...
int main(int argc, char* argv[])
{
    ...
    mreq.imr_interface.s_addr = inet_addr(argv[1]);//htonl(INADDR_ANY); /*网络接口为默认*/
                                                        /*将本机加入多播组*/
	...
	while(1)
	{
		...
		sendto(fd, "b", 1, 0, (struct sockaddr*)&multiaddr, SOCKADDR_LEN);
#ifdef WIN32		
		...
		if(ret > 0)
		{
			...
			sleep(1);
			sendto(fd, buf, ret, 0, (struct sockaddr*)&multiaddr, SOCKADDR_LEN);
		}
	}
